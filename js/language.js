...
function updateWhyUsSection(whyUsData) {
    ...
    if (valueCards.length === whyUsData.cards.length) {
        valueCards.forEach((card, index) => {
            ...
            if (description) description.textContent = whyUsData.cards[index].description;
        });
    }
}
...
function updateServicesSection(servicesData) {
    ...
    if (serviceCards.length === servicesData.cards.length) {
        serviceCards.forEach((card, index) => {
            ...
            if (description) description.textContent = servicesData.cards[index].description;
        });
    }
}
...
function updateProcessSection(processData) {
    ...
    const processSubtitle = document.querySelector('#process .section-header p:nth-child(2)');
    const processDescription = document.querySelector('#process .section-header p:nth-child(3)');
    ...
    if (processDescription) processDescription.textContent = processData.description;
    ...
    if (processSteps.length === processData.steps.length) {
        processSteps.forEach((step, index) => {
            ...
            if (description) description.textContent = processData.steps[index].description;
        });
    }
}
...
function updateGoalsSection(goalsData) {
    const goalsTitle = document.querySelector('#audience-goals .section-header h2');
...
function updateAudienceSection(audienceData) {
    const audienceTitle = document.querySelector('#audience-goals .section-header:nth-of-type(2) h2');
...
function updateGallerySection(galleryData) {
    ...
    if (galleryItems.length === galleryData.items.length) {
        galleryItems.forEach((item, index) => {
            ...
            if (description) description.textContent = galleryData.items[index].description;
        });
    }
}
...
function updateTestimonialsSection(testimonialsData) {
    ...
    if (testimonialsSubtitle) testimonialsSubtitle.textContent = testimonialsData.subtitle;
...
function updateContactSection(contactData) {
    ...
    const contactSubtitle = document.querySelector('#contact > .container > .cta-content > p');
...
function updateFooterSection(footerData) {
    ...
    // Update Contact section
    const contactSection = footerSections[2];
    if (contactSection) {
        const contactTitle = contactSection.querySelector('h3');
        const contactItems = contactSection.querySelectorAll('ul.contact-info li');
        
        if (contactTitle) contactTitle.textContent = footerData.contact.title;
        
        if (contactItems.length >= 3) {
            // Address
            const addressText = contactItems[0].childNodes[1];
            if (addressText) {
                addressText.textContent = ' ' + footerData.contact.address;
            }
            
            // Phone
            const phoneText = contactItems[1].childNodes[1];
            if (phoneText) {
                phoneText.textContent = ' ' + footerData.contact.phone;
            }
            
            // Email
            const emailText = contactItems[2].childNodes[1];
            if (emailText) {
                emailText.textContent = ' ' + footerData.contact.email;
            }
        }
    }
